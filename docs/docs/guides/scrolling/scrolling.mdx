...
On iOS, you can enable the `scrollable` prop to automatically pin scroll views (including `ScrollView` and `FlatList`) to fit within the sheet's available space.
...
const App = () => {
  ...
  return (
    <TrueSheet ref={sheet} scrollable={true}>
