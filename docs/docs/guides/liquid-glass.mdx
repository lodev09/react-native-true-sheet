...
TrueSheet's blur effect props work seamlessly with Liquid Glass on iOS 26+. You can use `backgroundBlur` and `blurOptions` to customize the blur appearance:
...
See the [Configuration reference](/reference/configuration#backgroundblur) for all blur options.
