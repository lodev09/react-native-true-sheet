...
Use `popTo` to dismiss sheets until reaching a specific screen, or `popToTop` to dismiss all sheets and return to the base screen:
...
function ProfileSheet() {
  ...
  return (
    <View>
      {/* Dismiss sheets until reaching 'Details' */}
      <Button title="Back to Details" onPress={() => navigation.popTo('Details')} />
      {/* Dismiss all sheets */}
...
With both methods, all sheets are dismissed in a single smooth animation rather than sequentially.
