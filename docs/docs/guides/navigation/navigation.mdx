...
| `reanimatedPositionChangeHandler` | `function` | A handler from `useReanimatedPositionChangeHandler` for position changes. |
...
:::note
When `reanimated: true` is set, `react-native-reanimated` must be installed. The integration is lazy-loaded, so screens without `reanimated: true` don't require reanimated.
:::
