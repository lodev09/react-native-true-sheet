...
:::info
iOS uses the native `UISheetPresentationController` grabber, while Android renders a native view following [Material Design 3 specifications](https://m3.material.io/components/bottom-sheets/specs) (32x4dp, centered, with the standard drag handle color).
:::
...
The intensity of the blur effect (0-100). Only applies when [`blurTint`](#blurtint) is set. Uses system default if not provided.
