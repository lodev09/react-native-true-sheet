...
Lazy-load the sheet's content on first `present()` call. When `true`, the native view is created only when `present()` is called. When `false`, the native view is created immediately on mount, making presentation instant.
...
:::info
When [`initialIndex`](#initialindex) is set, this prop is ignored and the sheet always renders immediately.
:::
