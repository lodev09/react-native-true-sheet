...
const App = () => {
  ...
  const handleOnWillPresent = (e: WillPresentEvent) => {
    console.log(e.nativeEvent) // { index: 0, value: 420, position: 123.5 }
  ...
  const handleDetentChange = (e: DetentChangeEvent) => {
    console.log(e.nativeEvent) // { index: 1, value: 680, position: 234.5 }
  }
...
This is called continuously when the sheet's position changes during drag operations. This event fires more frequently than `onDragChange` and provides real-time position updates.
