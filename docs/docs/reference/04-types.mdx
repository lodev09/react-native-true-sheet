...
| Property | Type | Description | Default |
| - | - | - | - |
| `contentInsetAdjustmentBehavior` | `boolean` | Applies the bottom safe-area inset to the scroll content automatically while the content can scroll. On iOS, sets the scroll view's native [`contentInsetAdjustmentBehavior`](https://developer.apple.com/documentation/uikit/uiscrollview/contentinsetadjustmentbehavior) to `automatic`; on Android and Web, the sheet applies the inset itself — parity React Native's iOS-only prop can't provide. Only applies while the sheet's [`insetAdjustment`](configuration#insetadjustment) is `'automatic'`; a relative footer absorbs the inset instead. | `true` |
