...
### 5. Native Bottom Safe Area Handling (iOS)

iOS now handles bottom safe area insets natively for the sheet. This means the sheet height includes the safe area automatically, providing consistent behavior across all Apple devices (iPhone, iPad, visionOS).

**What this means:**
- The `position`, `index`, and `detent` values are calculated purely from the actual sheet position relative to screen height
- On iPhone, a sheet at detent `0.5` will be taller than `0.5 * screenHeight` because iOS adds the bottom safe area internally
