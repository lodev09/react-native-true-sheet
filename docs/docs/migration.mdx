...
### 1. Background and Blur Behavior Changes
...
```tsx
// ❌ v2 - blurTint overrides backgroundColor
<TrueSheet backgroundColor="red" blurTint="light">
  {/* backgroundColor is ignored, only blur effect is shown */}
</TrueSheet>

// ✅ v3 - blurTint applies over backgroundColor
<TrueSheet backgroundColor="red" blurTint="light">
  {/* Shows red background with light blur effect on top */}
</TrueSheet>

// If you want only blur without background color:
<TrueSheet blurTint="light">
  {/* Just blur effect with system default background */}
</TrueSheet>
```

### 2. Fabric Architecture Required
...
<TrueSheet
  ...
  edgeToEdge
  onPresent={(e) => console.log('presented', e.nativeEvent)}
  onDismiss={() => console.log('dismissed')}
  onSizeChange={(e) => console.log('size changed', e.nativeEvent)}
>
  <View />
</TrueSheet>
...
<TrueSheet
  ...
  onDidPresent={(e) => console.log('presented', e.nativeEvent)}
  onDidDismiss={() => console.log('dismissed')}
  onDetentChange={(e) => console.log('detent changed', e.nativeEvent)}
>
  <View />
</TrueSheet>
```
...
:::tip
See the [Scrolling guide](/guides/scrolling) for more information about `fitScrollView`.
:::
...
Version 3 now includes first-class iPad support with the new `pageSizing` prop. When enabled (default), the sheet sizes itself appropriately for larger displays like iPad, providing a centered, page-like appearance instead of stretching to full width.
...
:::info
This feature is **iOS 17+ only** and defaults to `true`. Set to `false` if you want the sheet to stretch to full width on iPad.
:::
