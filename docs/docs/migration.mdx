...
The navigator is now built on [`standard-navigation`](https://github.com/react-navigation/standard-navigation), so one implementation works with both React Navigation and Expo Router. For React Navigation apps, the `/navigation` entry point now requires `@react-navigation/native` version `7.3.0` or higher (previously `@react-navigation/core`):

```sh
yarn add @react-navigation/native@^7.3.0
```
...
The `withLayoutContext` recipe is replaced by a ready-to-use `Sheet` layout. Requires Expo SDK 57+ — no `@react-navigation/*` install needed.
...
5. **Update your code:**
   ...
   - If using the Sheet Navigator, update `@react-navigation/native` to 7.3+
   - If using Expo Router, replace the `withLayoutContext` wrapper with the `Sheet` layout from `/navigation/expo-router` and update `useTrueSheetNavigation` imports
6. **Test your app:**
   ```sh
