...
To keep an absolute footer behind the keyboard instead, set `avoidKeyboard` to `false` — it stays pinned to the sheet's bottom edge and the keyboard covers it, like a relative footer. While the keyboard is open, the footer stops padding a plugged scrollable and counting toward the `auto` detent, so the content ends at the keyboard instead of leaving a footer-sized gap:
