...
<img alt="liquid-glass" src="/img/preview-ios.gif" width="300"/>
...
By default, TrueSheet enables Liquid Glass on iOS 26+ devices. The sheet will automatically display with the Liquid Glass effect unless [`glass`](../reference/configuration#glass) is `false` or [`backgroundBlur`](../reference/configuration#backgroundblur) is set.

## Tinting Liquid Glass

[`backgroundColor`](../reference/configuration#backgroundcolor) paints over the glass, so a translucent color tints it while keeping the blur and the content behind the sheet visible.
...
Set [`glass`](../reference/configuration#glass) to `false` to remove the glass behind [`backgroundColor`](../reference/configuration#backgroundcolor). Setting [`backgroundBlur`](../reference/configuration#backgroundblur) also removes it, since the blur replaces the glass material.
