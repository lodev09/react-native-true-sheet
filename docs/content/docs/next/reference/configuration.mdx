...
On iOS 26+, this paints over the [Liquid Glass](../guides/liquid-glass) background. Use a translucent color to tint the glass, or set [`glass`](#glass) to `false` for a flat color.
:::

## `glass`

Keeps the [Liquid Glass](../guides/liquid-glass) background behind [`backgroundColor`](#backgroundcolor). Set to `false` to remove the glass and render a flat background. [`backgroundBlur`](#backgroundblur) always removes the glass.
...
Anchors the sheet to the specified horizontal edge. `'leading'` and `'trailing'` follow the layout direction (left/right in LTR, right/left in RTL).
...
:::info
On iOS 27+, the sheet uses the native `preferredPlacement` API. Earlier versions use `sourceView`. System-defined margins prevent fully flush edge attachment.
:::
...
The offset from the screen edge. Only applies when [`anchor`](#anchor) is `'leading'` or `'trailing'`.
...
A ref to the scrollable component (e.g. `ScrollView`, `FlatList`) rendered within the sheet content. Required for scrollable handling — nested scrolling, keyboard insets, and `auto` detent sizing are wired to this scrollable. See [this guide](../guides/scrolling) for more information.
...
Options for customizing scrollable behavior. Applies to the scrollable provided via [`scrollableRef`](#scrollableref). See [this guide](../guides/scrolling) for more information.
