...
### 6. Scrollable Bottom Inset via `contentInsetAdjustmentBehavior`

A plugged scrollable still gets the bottom safe-area inset applied natively, now only while the content can actually scroll — mirroring iOS's [`contentInsetAdjustmentBehavior="automatic"`](https://developer.apple.com/documentation/uikit/uiscrollview/contentinsetadjustmentbehavior) on all platforms. Remove any manual safe-area padding from your scroll content, or opt out with the new `scrollableOptions.contentInsetAdjustmentBehavior` and pad it yourself:

```tsx
// ✅ v4 — the inset is applied natively while the content can scroll
...
:::info
If your sheet has a relative footer, it absorbs the bottom inset instead (see above) — pad for the footer's height, not the safe area.
:::
...
:::tip
See the [Navigation guide](guides/navigation#expo-router) for more details.
:::

### 9. `backgroundColor` No Longer Disables Liquid Glass

On iOS 26+, `backgroundColor` now paints over the [Liquid Glass](guides/liquid-glass) background instead of replacing it. Opaque colors look the same as before. A translucent color now tints the glass instead of rendering a see-through flat color. To get the v3 flat background, set the new `glass` prop to `false`.
...
### 10. `anchor` Uses `leading` / `trailing`

The `anchor` values `'left'` and `'right'` are renamed to `'leading'` and `'trailing'`. Native side sheets already followed the layout direction, so the new names match what the sheet does — `'leading'` is the left edge in LTR and the right edge in RTL. Web now follows the layout direction too.
...
:::tip
See the [Side Sheets guide](guides/side-sheets) for more details.
:::
...
The new `/navigation/expo-router` entry point exports a ready-to-use `Sheet` layout that integrates with Expo Router's built-in navigation directly. See the [Navigation guide](guides/navigation#expo-router).

### 5. Static API with `createTrueSheetScreen`

The navigator now supports React Navigation's [static API](https://reactnavigation.org/docs/static-configuration):
...
5. **Update your code:**
   ...
   - Rename `anchor="left"` / `anchor="right"` to `anchor="leading"` / `anchor="trailing"`
