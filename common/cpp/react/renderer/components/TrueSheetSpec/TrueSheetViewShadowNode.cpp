...
#if !defined(ANDROID)
void TrueSheetViewShadowNode::setEventDispatcher(std::weak_ptr<const EventDispatcher> dispatcher) {
  getStateDataMutable().setEventDispatcher(dispatcher);
}

TrueSheetViewShadowNode::StateData &TrueSheetViewShadowNode::getStateDataMutable() {
  ensureUnsealed();
